...
void Sciezka::WyswietlWspolrzedneFormatGnuplot(std::ostream& StrmWy) const
{
  ...
  for(int i=0;i<(Objekt_Graficzny::Rozmiar()+1);++i){
    Objekt_Graficzny::Wartosc_Glob(i>=Objekt_Graficzny::Rozmiar()?(i%Objekt_Graficzny::Rozmiar())+1:i).WyswietlWspolrzedneFormatGnuplot(StrmWy);
  }

  /*
  for (std::vector<Wektor2D>::const_iterator it = _punkty_posrednie.begin() ; it != _punkty_posrednie.end(); ++it)
    StrmWy<<*it<<std::endl; 
  */
}

void Sciezka::Przygotuj_Animacje_Sciezki(Robot& Rob,double dlugosc)
{
  ...
  /*
    std::cout<<"Animacja sciezki"<<Wektor<<std::endl;
    
    if(Objekt_Graficzny::Wartosc_Lok(Objekt_Graficzny::Rozmiar()-1).Odleglosc_Wektorow(Objekt_Graficzny::Wartosc_Lok(Objekt_Graficzny::Rozmiar()-1)+Wektor)>Objekt_Graficzny::Wartosc_Lok(Objekt_Graficzny::Rozmiar()-1).Odleglosc_Wektorow(Objekt_Graficzny::Wartosc_Lok(Objekt_Graficzny::Rozmiar()-1)-Wektor))
    //    Wektor=Macierz2x2(180)*Wektor;
    Wektor*=-1;
    std::cout<<"Animacja sciezki"<<Wektor<<std::endl;
  */  
