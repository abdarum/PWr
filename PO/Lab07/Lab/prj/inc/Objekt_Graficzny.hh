...
/*!
 * \brief Tryb w którym jest objekt
 *
 * Określa w którym trybie pracuje Obiekt_Graficzny
 */
enum Tryb_Pracy { Ob_Objekt_Graficzny, Ob_Robot, Ob_Sciezka, Ob_Przeszkoda}; 
...
public:
  ...
  Objekt_Graficzny(std::vector<double> Wspolrzedne_Wierzcholkow,const double &Kat_Obrotu){ _wsp_lokalne.Ustaw(Wspolrzedne_Wierzcholkow); _kat=Kat_Obrotu;}
  /*!
   * \brief Inicjalizacja zmiennej typu ZbiorWierzcholkow wartosciami
   *
   ...
   *                        domyślego położenia
   */
  Objekt_Graficzny(std::vector<Wektor2D> Wierzcholki,const double &Kat_Obrotu){_wsp_lokalne.Ustaw(Wierzcholki); _kat=Kat_Obrotu;}
  /*!
   * \brief Inicjalizacja zmiennej typu ZbiorWierzcholkow wartosciami
   *
   * \param[in] Wspolrzedne_Wierzcholkow ustawia parametry wspolrzednych
 ...
 /*!
   ...
   */
  virtual Tryb_Pracy Rodzaj_Obiektu(){return Ob_Objekt_Graficzny;}
  /*!
   * \brief Zwraca tryb pracy w którym pracuje Obiekt_Graficzny
   */
  virtual std::string Rodzaj_Obiektu_Nazwa() const {return "Objekt_Graficzny";}
  /*!
   * \brief Zwraca największą wspolrzędną X Objektu.
   *
   * Funkcja nie uwzględnia wektora globalnego.
   */
  double Max_X() const;
 /*!
   * \brief Zwraca najmniejszą wspolrzędną X Objektu.
   *
   * Funkcja nie uwzględnia wektora globalnego.
   */
  double Min_X() const;
 /*!
   * \brief Zwraca największą wspolrzędną Y Objektu.
   *
   * Funkcja nie uwzględnia wektora globalnego.
   */
  double Max_Y() const;
 /*!
   * \brief Zwraca najmniejszą wspolrzędną Y Objektu.
   *
   * Funkcja nie uwzględnia wektora globalnego.
   */
  double Min_Y() const;
  /*!
   * \brief Wyswietla skrajnepunkty Objektu.
   *
   * Funkcja nie uwzględnia wektora globalnego.
   */
  void Wyswietl_Ekstrema() const
  {
    std::cout<<std::setw(6)<<std::fixed<<std::setprecision(2)<<"Max X: "<<Max_X()<<"Min X: "<<Min_X()<<"Max Y: "<<Max_Y()<<"Min Y: "<<Min_Y()<<std::endl;
  }


  virtual Wektor2D Wektor_Translacji() const{return Wektor2D();}
  virtual void Wektor_Translacji(Wektor2D Wsp_Globalne){}
  virtual double Promien() const{return 0;}
  virtual unsigned int ID_Robota()const{return 0;}
