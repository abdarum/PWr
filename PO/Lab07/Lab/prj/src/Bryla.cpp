...
void Bryla::Ustaw(Wektor2D Wart[LICZBA_PUNKTOW])
{
...
void Bryla::ObrocOKat(Macierz2x2 Mac,int count)
{
  for(;count>0;--count)
    for(int i=0;i<LICZBA_PUNKTOW;++i)
      {
...
void Bryla::PrzesunOWektor(Wektor2D const Wek)
{
  for(int i=0;i<LICZBA_PUNKTOW;++i)
    _bryla[i]+=Wek;
}
