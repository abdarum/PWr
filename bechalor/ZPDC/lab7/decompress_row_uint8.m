function [ Y ] = decompress_row_uint8( X )
...
%   Detailed explanation goes here
    ...
    for i=1:length(X)
        ...
        if (counting_val == 0)
            tmp = zeros(unskip_LF_and_CR_uint8(X(i))+overleaf_const*overflow_cnt); tmp = tmp(1,:);
            Y = [Y, tmp];
            counting_val = 1;
            overflow_cnt = 0;
        else
            tmp = ones(unskip_LF_and_CR_uint8(X(i))+overleaf_const*overflow_cnt); tmp = tmp(1,:);
            Y = [Y, tmp];
            counting_val = 0;
            overflow_cnt = 0;
        end
