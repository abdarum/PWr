...
Program: przetwarzanie_obrazow.c ver. 1.1
...
void podaj_nazwe_pliku(FILE *plik_we,int obraz_pgm[][MAX],int wymx,int wymy, int szarosci,int odczytano){
  char nazwa[100];
  printf("Podaj nazwe pliku:\n");
  scanf("%s",nazwa);
  plik=fopen(nazwa,"r");
  *odczytano = czytaj(plik_we,obraz_pgm,wymx,wymy,odcieni);
  fclose(plik_we);
}
...
int main() {
  ...
  podaj_nazwe_pliku(plik,obraz,&wymx,&wymy,&odcieni,&odczytano);
...
Gdy funkcje przetwarzania obrazu byly przetestowane i dzialajace zajalem sie tworzeniem menu. Zaczalem od papisania wlasciwej czesci menu. Gdy ona dzialala zaczalem pisac obsluge bledow otwierania pliku.
