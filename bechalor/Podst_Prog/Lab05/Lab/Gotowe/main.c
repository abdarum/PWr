...
int main(int argc,char **argv){
  ...
  odczytano=czytaj_wymiary(opcje->plik_we,&obraz);
  czytaj(opcje->plik_we,&obraz);
  fclose(opcje->plik_we);
  if((opcje->rgbw).bw==1)
    zrob_czarno_bialy(&obraz);
 
  if(odczytano == 0|| odczytano==B_BRAKPLIKU)
    bledy_opcji=B_BRAKPLIKU;
...
- gdy funkcje w pelni dzialaly przystapilem do dopisania opcji m.
  ...
  Powstaja bledy ktorych nie jestem w stanie wyeliminowac poniewaz 
  nie jestem w stanie utworzyc dynamicznych tablic i zmiennych
  tak zeby byla stala wartosc wymx
  Przykladowy blad:

     przetwarzanie.c: In function ‘negatyw’:
     przetwarzanie.c:16:3: warning: ISO C90 forbids variable length array ‘tablica’ [-Wvla]
        int (*tablica)[obraz->wymx];
	^

  dlatego ostatecznie kompiluje z flagami:
    gcc -pedantic -Wall -Wno-vla
