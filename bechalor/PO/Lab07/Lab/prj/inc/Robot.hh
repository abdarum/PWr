...
{
  // ustawiony środek ciężkości
  std::vector<double> Domyslne_Wartosci{5,0, 15,0, 10.6,-10.6, -10.6,-10.6, -10.6,10.6, 10.6,10.6};
...
public:
   ...
   *          translacji
   */
  Robot(Wektor2D Wsp_Globalne,double Kat_Obrotu,unsigned int ID){Objekt_Graficzny(); Objekt_Graficzny::Ustaw(Domyslne_Wartosci); _wektor_translacji=Wsp_Globalne; Objekt_Graficzny::Kat(Kat_Obrotu); _id_robota=ID;
    if(abs(Kat_Obrotu)>std::numeric_limits<double>::epsilon()) ObrocOKat(0);}
  /*!
...
bool Kolizja(const  std::list<std::shared_ptr<Objekt_Graficzny>> &objekty,const Wektor2D &Obecne_Polozenie) const;
  /*!
   * \brief Zwraca ID robota.
   */
  unsigned int ID_Robota()const{return _id_robota;}
  /*!
   * \brief Zwraca ID robota.
   */
  void ID_Robota(int Ile){_id_robota=Ile;}
