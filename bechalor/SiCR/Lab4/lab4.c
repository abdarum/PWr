...
void reg_trojstanowy(int chan, int val){
	if (chan == 1){
		cool_temp = val;
	} else if (chan == 2){
		heat_temp = val;
...
//	GPIO0 _0 _1
	if ((current_temp >= heat_temp) && (current_temp <= cool_temp)){
//		nic
//		DOUT 0 0
        out8(GPIO0_DATA,0);
	} else if(current_temp < heat_temp) {
//		grzanie
//		DOUT 0 1
        out8(GPIO0_DATA,2);
	} else if(current_temp > cool_temp) {
//		chlodzenie
//		DOUT 1 0
        out8(GPIO0_DATA,1);
	}
...
#define WY 0xFF
   ...
   printf("test 2 \n");
   base2 = mmap_device_io(20,GPIO0_DATA);
   printf("test 3 \n");
   out8(GPIO0_DIR, 0xFF);
   printf("test 4 \n");
   card_init(0,3,5);
