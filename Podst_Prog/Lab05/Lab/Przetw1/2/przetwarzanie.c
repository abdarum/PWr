...
void gamma(obraz_struct *obraz,double gamma){
  ...
  for(i=0;i<obraz->wymy;i++){
    for(j=0;j<obraz->wymx;j++){ 
       tablica[i][j] =(int)(obraz->szarosci * pow(tablica[i][j]/obraz->szarosci, gamma_tmp));
    }
  }
} 
*/
...
/*      tablica[i][j] = (int)(pow(tablica[i][j],(1/gamma))/pow(obraz->szarosci,((1-gamma)/gamma))); 
tablica[i][j] = (int)(pow(tablica[i][j],(1/gamma))/down); */
...
void gamma(obraz_struct *obraz,double gamma){
  ...
  double tmp_tab[obraz->wymy][obraz->wymx];
  for(i=0;i<obraz->wymy;i++){
    for(j=0;j<obraz->wymx;j++){
      tmp_tab[i][j] = tablica[i][j];
    }
  ...
  double tmp_szarosci;
  tmp_szarosci = obraz->szarosci;  

  float gamma_tmp;
  gamma_tmp = (1- gamma);
  for(i=0;i<obraz->wymy;i++){
    for(j=0;j<obraz->wymx;j++){ 
tablica[i][j] =(int)(tmp_szarosci * pow((tmp_tab[i][j]/tmp_szarosci), gamma_tmp));
    }
  }
}
...
void gamma(obraz_struct *obraz,double gamma){
  ...
  for(i=0;i<obraz->wymy;i++){
    for(j=0;j<obraz->wymx;j++){ 
tablica[i][j] =tmp_szarosci * pow((tmp_tab[i][j]/tmp_szarosci), 1/gamma);
    }
  }
}
